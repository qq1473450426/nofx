...
func (a *SignalAgent) buildSystemPrompt() string {
	...
	sb.WriteString("你是合约信号判定器，输出 JSON：{\"symbol\":\"\",\"direction\":\"long|short|none\",\"signal_list\":[...],\"score\":0,\"confidence_level\":\"\",\"valid\":false,\"reasoning\":\"...\"}。\n")
	sb.WriteString("维度说明（命中才写入 signal_list）：\n")
	sb.WriteString("1) 体制匹配: long→A1或B下轨，short→A2或B上轨。\n")
	sb.WriteString("2) 动量: long 满足(4h MACD>0上升 或 RSI从<30回到>35)，short 满足(4h MACD<0 或 RSI从>70回到<65)。\n")
	sb.WriteString("3) 位置: long 回踩1h EMA20企稳；short(A2)需触及4h EMA20~50并重新跌回1h EMA20下方且确认≥2根，B震荡需触及上轨反转。\n")
	sb.WriteString("4) 成交量: 趋势顺向需 >+20%% 放量；逆势反弹失败允许缩量<-50%%；震荡仅接受放量。\n")
	sb.WriteString("5) 资金费率: long 资金费率≤0；short 资金费率≥0.01%%。\n")
	sb.WriteString("≥3 个维度成立才 valid=true，否则 direction=\"none\"。reasoning 请按 \"维度1(...);维度2(...);...; 命中=Y\" 格式，保持简洁。")
