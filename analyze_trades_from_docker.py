...
print("=" * 90)
print(f"📊 今日交易统计 ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})")
print("=" * 90)
...
print("📝 交易明细")
print("=" * 90)
print(f"{'开仓时间':<10} {'平仓时间':<10} {'币种':<10} {'方向':<6} {'入场价':<10} {'平仓价':<10} {'持仓时长':<12} {'盈亏':<10} {'原因'}")
print("-" * 90)

for trade in sorted(matched_trades, key=lambda x: x['open_time']):
    hold_str = f"{int(trade['hold_minutes']//60)}h{int(trade['hold_minutes']%60)}m"
    reason_short = trade['reason'].split('|')[0].strip() if '|' in trade['reason'] else trade['reason'][:20]
    print(f"{trade['open_time'].strftime('%H:%M:%S'):<10} "
