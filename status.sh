...
# 检查决策日志
echo "📊 决策日志:"
DECISION_COUNT=$(find decision_logs -name "decision_*.json" -type f 2>/dev/null | wc -l | tr -d ' ')
if [ "$DECISION_COUNT" -gt 0 ]; then
    echo "   📝 总决策数: $DECISION_COUNT"
    
    LATEST=$(find decision_logs -name "decision_*.json" -type f 2>/dev/null | sort -r | sed -n 1p)
    if [ -n "$LATEST" ]; then
